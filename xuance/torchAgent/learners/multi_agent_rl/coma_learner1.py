...
class COMA_Learner(LearnerMAS):
    ...
    def update(self, sample, epsilon=0.0):
        ...
        returns = torch.Tensor(sample['returns']).squeeze(-1).to(self.device)
        ...
        new_returns = new_returns.unsqueeze(-1).expand(-1, -1, self.n_agents, -1).clone().squeeze(-1)
        ...
        if self.use_global_state:
            ...
            critic_in = torch.concat([obs, actions_in], dim=-1)

        # 获取 Critic 值
        _, q_eval = self.policy.get_values(critic_in)
        q_eval_a = q_eval.gather(-1, actions.unsqueeze(-1).long()).squeeze(-1)
        q_eval_a *= agent_mask
        new_returns *= agent_mask
        loss_c = ((q_eval_a - new_returns.detach()) ** 2).sum() / agent_mask.sum()
    ...
    def update_recurrent(self, sample, epsilon=0.0):
        ...
        td_errors = q_eval_a - new_returns.detach()
        td_errors *= filled_n
